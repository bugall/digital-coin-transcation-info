...
class Controller {
  ...
  async getExchangePrice(page) {
    const [err, data] = await to(page.evaluate(() => document.querySelector('#market_depth').innerText))
    if (err) this.retryLaunchPage()
    return data
  }
  async getExchangeInfo(page) {
    const [err, data] = await to(page.evaluate(() => document.querySelector('#market_trades_list').innerText))
    if (err) this.retryLaunchPage()
    return data
  }
